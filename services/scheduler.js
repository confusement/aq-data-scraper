...
async function startJobs(){
    while(true){
        ...
        for(j of jobs){
            if(j.stages.acquisition.status!="S" && j.stages.acquisition.retriesLeft>=0){
                if(browserLoad<browserLoadLimit){
                    runQueue.push(cpcb.acquisition(j.id,j.stages.acquisition.data));
                    browserLoad+=1;
                }
            }
            else if((j.stages.preprocess.status!="S" && j.stages.preprocess.retriesLeft>=0)){
                runQueue.push(cpcb.preprocess(j.id,j.acquisition.data));
            }
            else if((j.stages.mapOntology.status!="S" && j.stages.mapOntology.retriesLeft>=0)){
